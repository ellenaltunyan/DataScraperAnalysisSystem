...
public class news_am_scraper {
    
    private static final Gson gson = new Gson();
    String resourcePath = Paths.get(System.getProperty("user.dir"), "src", "resources", "news_am.json").toString();

    // public static void main(String[] args) throws InterruptedException {
    //     List<Map<String, String>> scrapedDataList = new ArrayList<>();

    //     // Scrape and accumulate data in the list
    //     for (int i = 4620; i > 4600; i--) {
    //         Thread.sleep(5000);
    ...
    private static Map<String, String> scrapeData(int id) {
        try {
            String url = String.format("https://tech.news.am/eng/news/%s/", id);
